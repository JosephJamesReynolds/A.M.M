...
contract AMM {
  ...
  function addLiquidity(uint256 _token1Amount, uint256 _token2Amount) external {
    ...
    // If first time adding liquitidy , make share 100
    if (totalShares == 0) {
      share = 100 * PRECISION;
    } else {
  ...
  // Determine how many token2 tokens must be deposited when depositing liquitidy for token1 
  function calculateToken2Deposit(uint256 _token1Amount) public view returns (uint256 token2Amount) {
    token2Amount = (token2Balance * _token1Amount) / token1Balance;
  }  

  // Determine how many token1 tokens must be deposited when depositing liquitidy for token2
  ...
  function calculateToken2Swap(uint256 _token2Amount) public view returns (uint256 token1Amount) 
  {
    ...
    // Do Swap
    // 2. transfer token2 tokens out of user wallet
    token2.transferFrom(msg.sender, address(this), _token2Amount);
    // 2. Update the token2 balance in the contract
    token2Balance += _token2Amount;
    // 3. Update the token1balance in the contract
