...
describe("Token", () => {
  ...
  describe("Swapping tokens", () => {
    ...
    it("facilitates swaps", async () => {
      ...
      // Check the K value
      expectedK = amount.mul(amount);
      actualK = await amm.K();
      expect(actualK).to.equal(expectedK);
      // console.log("K: ", actualK.toString());
      ...
      // Investor1 approves all (100k) Tokens
      transaction = await token1
        .connect(investor1)
      ...
      // Investor2 approves all (100k) Tokens
      transaction = await token2
        .connect(investor2)
        .approve(amm.address, tokens(100000));
      await transaction.wait();
      ...
      console.log(
        `AMM Token 1 Balance: ${ethers.utils.formatEther(
          await amm.token1Balance()
        )}`
      );
      console.log(
        `AMM Token 2 Balance: ${ethers.utils.formatEther(
          await amm.token2Balance()
        )}`
      );
